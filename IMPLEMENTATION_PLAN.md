...
18. ~~**Create `agent_assisted_3d.gd`** — `class_name AgentAssisted3D extends Node3D`~~ ✅
19. ~~**Add exposed properties** — `prompt` (String, multiline setter with hash tracking), `texture_attachments` (Array[Texture2D]), `generation_status` (enum), `status_message`~~ ✅
20. ~~**Add API override properties** — `api_endpoint`, `api_key`, `model` (empty = use project settings)~~ ✅
21. ~~**Implement `_ready()` lifecycle** — Check for existing children (scene restore), check cache, auto-generate if prompt is non-empty~~ ✅
22. ~~**Implement generation pipeline with error correction loop `generate()`** — `MAX_RETRIES` (5) loop: PromptBuilder.build() → AIClient.chat() → ScriptExecutor.execute_with_error() → on error, PromptBuilder.build_error_correction() appends error to messages and retries; on success, _save_cache() → status update, emits `generation_started` / `generation_finished` signals~~ ✅
23. ~~**Implement prompt change detection** — `_prompt_changed()` with hash comparison, `_on_prompt_changed()` invalidates cache and triggers generate~~ ✅
24. ~~**Implement `force_generate()`** — Bypasses prompt hash check, always triggers fresh AI call~~ ✅
...
34. ~~**Implement cache directory structure** — `res://generated/<instance_id>/` with `current.gd` symlink pattern~~ ✅
35. ~~**Implement instance ID generation** — `_get_scene_path_hash()` using hash of scene path + node path~~ ✅
36. ~~**Implement `_save_cache()`** — Creates `GDScript` resource from `script_text`, sets prompt hash metadata, saves via `ResourceSaver`~~ ✅
37. ~~**Implement `_has_valid_cache()`** — Validates file existence, loads as `GDScript`, compares prompt hash metadata~~ ✅
38. ~~**Implement `_load_from_cache()`** — Loads cached script, runs via `ScriptExecutor`, applies nodes as children~~ ✅
39. ~~**Implement `_invalidate_cache()`** — Deletes `current.gd` cache file on prompt change or force-regenerate~~ ✅
40. **Implement scene child persistence** — `_apply_generated_nodes()` adds children with `set_owner()`, auto-saves scene
